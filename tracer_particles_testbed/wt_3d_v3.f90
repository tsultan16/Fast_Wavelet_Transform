...
TYPE wbin
    ...
    INTEGER :: wcount = 0
    INTEGER :: wavelet_index(1:max_per_bin) = 0
    REAL :: amplitude(1:max_per_bin) = 0.0
    LOGICAL :: picked_up(1:max_per_bin) = .FALSE.

END TYPE wbin 

TYPE list_node

    INTEGER :: wavelet_index 
...
INTEGER, PARAMETER :: bin_width = 4
INTEGER, PARAMETER :: max_wavelets  = 100000
INTEGER, PARAMETER :: max_recon = 6
INTEGER, PARAMETER :: num_tracers = 1000 
INTEGER, PARAMETER :: bin_neighborhood  = 1
...
REAL*8 :: t1, t2, p(3)
REAL*4 :: wave_x(3), wave_x2(3), wave_x3(3), wave_x4(3) 
INTEGER :: x_bins, x(nx,nx,nx,3), tracer_loc(num_tracers,3), tracer_bin(num_tracers,3), tracer_wavelet(max_wavelets,2), wcount
...
wave_x2 = (/ 0.75*nx, 0.75*nx, 0.75*nx/) 
wave_x3 = (/ 0.75*nx, 0.25*nx, 0.65*nx/) 
wave_x4 = (/ 0.25*nx, 0.75*nx, 0.25*nx/) 
...
DO i = 1, nx
    dat_in(i,j,k) = 100.0 * SIN(8.0*(TWOPI/nx)*i) * EXP( -(SNGL(k-wave_x(3))/SNGL(nx/16))**2 - (SNGL(j-wave_x(2))/SNGL(nx/16))**2 - (SNGL(i-wave_x(1))/SNGL(nx/16))**2 ) + &
                    200.0 * SIN(8.0*(TWOPI/nx)*j) * EXP( -(SNGL(k-wave_x2(3))/SNGL(nx/16))**2 - (SNGL(j-wave_x2(2))/SNGL(nx/16))**2 - (SNGL(i-wave_x2(1))/SNGL(nx/16))**2 ) + &
                    150.0 * SIN(4.0*(TWOPI/nx)*i + 8.0*(TWOPI/nx)*j) * EXP( -(SNGL(k-wave_x3(3))/SNGL(nx/8))**2 - (SNGL(j-wave_x3(2))/SNGL(nx/8))**2 - (SNGL(i-wave_x3(1))/SNGL(nx/8))**2 ) + &
                    250.0 * SIN(16.0*(TWOPI/nx)*i + 8.0*(TWOPI/nx)*j) * EXP( -(SNGL(k-wave_x4(3))/SNGL(nx/20))**2 - (SNGL(j-wave_x4(2))/SNGL(nx/20))**2 - (SNGL(i-wave_x4(1))/SNGL(nx/20))**2 )                                         
END DO
END DO
END DO
...
DO i = 1, num_tracers
    ...
    tracer_loc(i,1) =  nx * p(1) !wave_x2(1) + 2 * p(1)
    tracer_loc(i,2) =  nx * p(2) !wave_x2(2) + 2 * p(2)
    tracer_loc(i,3) =  nx * p(3) !wave_x2(3) + 2 * p(3) 
    tracer_bin(i,1) = 1 + (tracer_loc(i,1)-1)/bin_width
    tracer_bin(i,2) = 1 + (tracer_loc(i,2)-1)/bin_width
    tracer_bin(i,3) = 1 + (tracer_loc(i,3)-1)/bin_width 
END DO
...
CALL tracer_wavelet_reconstruction()
...
SUBROUTINE sort_wavelet_bin(bin)

    TYPE(wbin), INTENT(INOUT) :: bin

    CALL sort(bin%wavelet_index,bin%amplitude,1,max_per_bin) 

END SUBROUTINE sort_wavelet_bin


RECURSIVE SUBROUTINE sort(arr1, arr2, low, hi)

    INTEGER, INTENT(INOUT) :: arr1(:)
    REAL, INTENT(INOUT) :: arr2(:)
    ...
    IF(low .LT. hi) THEN
        ...
        CALL partition(arr1, arr2, low, hi, ip)
    
        ! apply quicksort to each of the two partitions
        CALL sort(arr1, arr2, low, ip-1) ! lower than pivot
        CALL sort(arr1, arr2, ip+1, hi)  ! higer than pivot
...
SUBROUTINE partition(arr1, arr2, low, hi, ip)
    ...
    DO kbin = 1, x_bins 
        DO jbin = 1, x_bins 
            DO ibin = 1, x_bins
                ...
                CALL sort_wavelet_bin(wavelet_bin(ibin,jbin,kbin))
...
SUBROUTINE tracer_wavelet_reconstruction()

    REAL :: fx(nx,nx,nx), fw(nx,nx,nx), amp
    INTEGER :: i, j, k, ibin, jbin, kbin, ix, n, wc, tr
    TYPE(list_node), TARGET :: head, tail
    TYPE(list_node), POINTER :: tmp, new
    LOGICAL :: add_wavelet

    head%wavelet_index = -1
    tail%wavelet_index = -1
    ...
    PRINT*,'Computing tracer wavelet reconstruction..'
    ...
    DO tr = 1, num_tracers
        ...
        PRINT*,'Adding reconstruction contributions from tracer#, location = ',tr,tracer_loc(tr,:)
        ...
        DO kbin = 1, x_bins
            DO jbin = 1, x_bins 
                DO ibin = 1, x_bins
                    ...
                    ! add the wavelets from neighborhood bins to tracer reconstruction
                    IF((ABS(tracer_bin(tr,1)-ibin) .LE. bin_neighborhood) .AND. (ABS(tracer_bin(tr,2)-jbin) .LE. bin_neighborhood) &
                     .AND. (ABS(tracer_bin(tr,3)-kbin) .LE. bin_neighborhood)) THEN
                    
                        ! add wavelet contribution 
                        DO n = 0, MIN(wavelet_bin(ibin,jbin,kbin)%wcount,max_recon)-1
                    
                            ix = wavelet_bin(ibin,jbin,kbin)%wavelet_index(max_per_bin-n)
                            amp = wavelet_bin(ibin,jbin,kbin)%amplitude(max_per_bin-n)

                            ! check if we've already included this wavelet
                            IF(.NOT. wavelet_bin(ibin,jbin,kbin)%picked_up(max_per_bin-n)) THEN

                                wavelet_bin(ibin,jbin,kbin)%picked_up(max_per_bin-n) = .TRUE.
                                add_wavelet = .TRUE.
    ...
    ! reset wavelet pick-up flags in all bins
    DO kbin = 1, x_bins
        DO jbin = 1, x_bins 
            DO ibin = 1, x_bins
                
                wavelet_bin(ibin,jbin,kbin)%picked_up(:) = .FALSE.
    ...
    PRINT*,'Total number of wavelets used for reconstruction = ',wc
